<template>
  <main>
    ...
    <section>
      ...
      <p v-if="slot">
        Currently selected: {{ new Date(slot.startTime).toDateString() }}
        {{ new Date(slot.startTime).toTimeString() }} to
        {{ new Date(slot.endTime).toDateString() }}
        {{ new Date(slot.endTime).toTimeString() }}
      </p>
      <p v-else>Click on your desired slot below</p>
      <article v-for="slot in slots">
        <p @click="assignSlot(slot._id, slot)">
